...
class Customerreturndetail extends StatefulWidget {
  ...
  const Customerreturndetail({super.key});

  @override
  State<Customerreturndetail> createState() => _CustomerreturndetailState();
}
...
class _CustomerreturndetailState extends State<Customerreturndetail> {
                      ...
                      child: FittedBox(
                        ...
                        child: DropdownButton(
                          ...
                          onChanged: (String? newValue) async {
                            ...
                            if (newValue.toString().toLowerCase() == 'expired') {
                              print('is this working');
                              await SaleskHistory.updateSaleItem(
                                  data['icode'], 'selectedType', 'Expired');
  ...
  void _calculateTotal({num? qty}) {
    for (int i = 0; i < stocks.length; i++) {
      if (selectedTypeData[i] == "Expired" || _ifVat == 0) {
        tax = tax;
      } else {
        tax = tax +
  ...
  Future<void> _sendRequest() async {
    ...
    dynamic resJson = await api.sendData(
        '/api/vansales_return.store', AppState().token, jsonEncode(bodyJson));
  ...
  _getTypes() async {
    RestDatasource api = RestDatasource();
    stocks = await SaleskHistory.getSalesHistory();
    Map<String, dynamic> resJson = await api.getDetails(
      '/api/get_product_return_type?store_id=${AppState().storeId}',
        null);
